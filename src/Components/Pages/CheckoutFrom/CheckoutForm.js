...
const CheckoutForm = ({ price, clientSecret }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (intentError) {
      ...
      axios.put(
        `https://drone-house-server.adaptable.app/payment/${user.email}`,
        {
  ...
  return (
    <div className="text-center">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {payStatus === true ? (
          <Button
            className="hover:border-gray-400 bg-rose-600 hover:text-gray-500 w-full"
            sx={{
              mt: 2,
              color: "black",
              border: "1px solid",
          ...
          <Button
            ...
            type="submit"
            disabled={!stripe || payStatus === "success"}
          >
            Pay ${price}
          </Button>
        )}
