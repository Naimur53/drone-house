...
const Purchase = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://drone-house-server.adaptable.app/drones/${_id}`)
      .then((result) => {
        setData(result.data);
  ...
  const onSubmit = (orderInfo) => {
    ...
    axios
      .post("https://drone-house-server.adaptable.app/purchase", orderInfo)
      .then((res) => {
    ...
    toast.success(
      <div className="py-5">
        ...
        <h2 className="text-gray-500">
          {" "}
          For paying go to{" "}
          <NavLink className="underline text-blue-400" to="/dashboard/pay">
            pay page
          </NavLink>
        </h2>
      </div>,
    ...
    <Container maxWidth="xl">
      ...
      <Grid sx={{ mt: 5 }} container spacing={5}>
          ...
          <Grid container>
            ...
            <Grid item sm={12} md={6}>
              <div className="font-poppins">
                ...
                <h2 className="text-3xl text-yellow-500 underline mb-5">
                  ${data?.price}
                </h2>
                <h2 className="text-small text-gray-700">
                  {data?.description}
                </h2>
              </div>
            </Grid>
        ...
        <Grid item xs={12} md={4}>
          {data.name && (
            <form
              className="shadow-lg flex flex-col justify-center items-center py-4"
              onSubmit={handleSubmit(onSubmit)}
            >
              <Typography className="font-poppins" variant="h5" gutterBottom>
                Fill The From
              </Typography>
              <TextField
                sx={{ width: "75%", my: 2 }}
